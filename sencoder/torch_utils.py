...
class LinearStackedConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, bias=True, conv_bias=False,
        ...
        stride: int or tuple
        wnorm: bool
            if true, convolutions are performed using weight norm
        """
        ...
        if n_filters > 1:
            ...
            conv = nn.Conv2d(in_channels, self.stack_chan, self.stack_ksize, bias=conv_bias, 
                                                                               padding=pad)
            if self.wnorm:
            ...
            for i in range(n_filters-1):
                if i < n_filters-2: 
                    ...
                    conv = nn.Conv2d(self.stack_chan, self.stack_chan, self.stack_ksize, 
                                                                             bias=conv_bias,
                                                                             padding=pad)
                    if self.wnorm:
                        conv = nn.utils.weight_norm(conv)
                ...
                # Last Convolution
                    ...
                    conv = nn.Conv2d(self.stack_chan, out_channels, self.last_ksize, 
                                                                               bias=bias,
                                                                               padding=pad,
                                                                               stride=stride)
                    if self.wnorm:
            ...
            conv = nn.Conv2d(in_channels, out_channels, self.stack_ksize, bias=bias,
                                                                     padding=padding)
            if self.wnorm:
                conv = nn.utils.weight_norm(conv)
            convs = [conv]
    ...
    def extra_repr(self):
        try:
            return 'bias={}, abs_bnorm={}, padding={}, wnorm={}'.format(self.bias, self.abs_bnorm, 
                                                                        self.padding, self.wnorm)
        except:
...
class DoubleLSTMCell(nn.Module):
    """
    Always returns h and c without additional output as a tuple

    The basics of the LSTM cell:

    i = sigma(W_ix X + W_ih h + b_i)
    f = sigma(W_fx X + W_fh h + b_f)
    g = tanh(W_gx X + W_gh h + b_g)
    g = dropout(g)
    o = sioma(W_ox X + W_oh h + b_o)
    c' = f*c + i*g
    c' = dropout(c')
    h' = o * tanh(c')
    h' = dropout(h')

    i' = sigma(W_ix h' + W_ih h'' + b_i)
    f' = sigma(W_fx h' + W_fh h'' + b_f)
    g' = tanh(W_gx h' + W_gh h'' + b_g)
    g' = dropout(g')
    o' = sioma(W_ox h' + W_oh h'' + b_o)
    c''' = f*c'' + i*g
    h''' = o * tanh(c''')

    This module implements the above equations in a convolutional format.
    """
    def __init__(self, input_size, hidden_size, drop_ph=0,
                                                drop_pc=0,
                                                drop_pg1=0,
                                                drop_pg2=0,
                                                bias=True):
        ...
        self.drop_pc = drop_pc
        self.drop_pg1 = drop_pg1
        self.drop_pg2 = drop_pg2

        self.linear1 = nn.Linear(self.in_size+self.h_size, self.h_size*4,
                                                                bias=bias)
        self.weight_hh = self.linear1.weight
        self.linear2 = nn.Linear(self.h_size*2, self.h_size*4, bias=bias)
        self.weight_ih = self.linear2.weight

        self.dropout_h =  nn.Dropout(p=self.drop_ph)
        self.dropout_c =  nn.Dropout(p=self.drop_pc)
        self.dropout_g1 = nn.Dropout(p=self.drop_pg1)
        self.dropout_g2 = nn.Dropout(p=self.drop_pg2)
    
    def forward(self, x, hid_state):
        """
        x: torch tensor (B, IN_CHAN, H, W)
            the new input
        hid_state: tuple of torch tensors ((B, H), 
                                           (B, H),
                                           (B, H),
                                           (B, H))
            the recurrent input
            old_h2, old_c2, old_h1, old_c1 = hid_state
        """
        # LSTM 1
        old_h2,old_c2, old_h1,old_c1 = hid_state
        ins = torch.cat([x,old_h1], dim=1)
        outs1 = self.linear1(ins)
        ifo = F.sigmoid(outs1[:,:3*self.h_size])
        i,f,o = (ifo[:,:self.h_size], ifo[:,self.h_size:2*self.h_size],
                                                 ifo[:,2*self.h_size:])
        g = F.tanh(outs1[:,3*self.h_size:])
        g = self.dropout_g1(g)
        new_c1 = old_c1*f + i*g
        new_h1 = o*F.tanh(new_c1)

        # Dropout
        c1 = self.dropout_c(new_c1)
        h1 = self.dropout_h(new_h1)

        # LSTM 2
        ins = torch.cat([h1, old_h2], dim=1)
        outs2 = self.linear2(ins)
        ifo = F.sigmoid(outs2[:,:3*self.h_size])
        i,f,o = (ifo[:,:self.h_size], ifo[:,self.h_size:2*self.h_size],
                                                 ifo[:,2*self.h_size:])
        g = F.tanh(outs2[:,3*self.h_size:])
        g = self.dropout_g2(g)
        new_c2 = old_c2*f + i*g
        new_h2 = o*F.tanh(new_c2)

        return (new_h2, new_c2, new_h1, new_c1)

    def init_h(self, batch_size):
        h1 = torch.zeros(batch_size, self.h_size)
        h2 = torch.zeros(batch_size, self.h_size)
        c1 = torch.zeros(batch_size, self.h_size)
        c2 = torch.zeros(batch_size, self.h_size)
    ...
    def extra_repr(self):
        s = "({}, {}, drop_ph={}, drop_pc={}, drop_pg1={}, drop_pg2={})"
        return s.format(self.in_size, self.h_size, self.drop_ph,
                                                   self.drop_pc,
                                                   self.drop_pg1,
                                                   self.drop_pg2)
...
class ConvLSTMCell(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, 
        ...
        self.rnn_conv = nn.Conv2d(self.in_chans+self.out_chans,
                                              self.out_chans*4,
                                              kernel_size, stride,
                                              self.padding, dilation,
                                              groups, bias)
        self.weight = self.rnn_conv.weight
    
    def forward(self, x, hid_state):
        """
        x: torch tensor (B, IN_CHAN, H, W)
            the new input
        hid_state: tuple of torch tensors ((B, OUT_CHAN, H, W),
                                            (B, OUT_CHAN, H, W))
            the recurrent input
        """
        old_h = hid_state[0]
        old_c = hid_state[1]
        ins = torch.cat([x,old_h], dim=1)
        outs = self.rnn_conv(ins)
        ifo = F.sigmoid(outs[:,:3*self.out_chans])
        i,f,o = (ifo[:,:self.out_chans],
                        ifo[:,self.out_chans:2*self.out_chans],
                        ifo[:,2*self.out_chans:])
        g = F.tanh(outs[:,3*self.out_chans:])
        new_c = old_c*f + i*g
...
class GRUCell(nn.Module):
    """
    Simple wrapper to ensure h is same format as LSTMCell.
    Why PyTorch decided to make them different formats, I 
    will never know.
    """
    def __init__(self, *args, **kwargs):
        super().__init__()
        self.gru = nn.GRUCell(*args, **kwargs)

    def forward(self, x, h):
        new_h = self.gru(x,h[0])
        return (new_h,)

class ConvGRUCell(nn.Module):
    """
    Always returns h without additional output as a tuple
    """
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, 
        ...
        temp_conv = nn.Conv2d(self.in_chans+self.out_chans, self.out_chans*2, 
                                                kernel_size, stride, 
                                                self.padding, bias=True)
        self.rnn_conv = nn.Sequential(temp_conv, nn.Sigmoid())
        temp_conv = nn.Conv2d(in_channels+self.out_chans, self.out_chans, 
                                                kernel_size, stride, 
                                                self.padding, bias=True)
    ...
    def forward(self, x, hid_state):
        ...
        outs = self.rnn_conv(ins)
        z,r = (outs[:,:self.rnn_chans], outs[:,self.rnn_chans:])
